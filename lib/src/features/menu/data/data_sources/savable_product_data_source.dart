...
abstract interface class ISavableProductsDataSource implements IProductsDataSource {
...
final class DbProductsSource implements ISavableProductsDataSource {
  ...
  Future<List<Product>> fetchProducts({required int categoryId, int page = 0, int limit = 25}) async {
    ...
    ..where((u) => u.id.isBiggerThanValue(limit * page) & u.categoryId.equals(categoryId))
    ..limit(limit)).get();
    for (var elem in result) {
      final cat = await (_db.select(_db.category)..where((u) => u.id.equals(elem.categoryId))).getSingle();
      data.add(Product(
        id: elem.id,
        name: elem.name,
