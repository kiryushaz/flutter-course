...
final class DbLocationsDataSource implements ISavableLocationsDataSource {
  ...
  Future<List<Location>> fetchLocations({int page = 0, int limit = 25}) async {
    final result = await (_db.select(_db.location)
          ..where((u) => u.id.isBiggerThanValue(limit * page))
          ..limit(limit))
        .get();
    return List<Location>.of(result.map(
        (e) => Location(id: e.id, address: e.address, lat: e.lat, lng: e.lng)));
  }

  @override
  Future<void> saveLocations({required List<Location> locations}) async {
    for (var location in locations) {
      _db.into(_db.location).insertOnConflictUpdate(db.LocationCompanion.insert(
